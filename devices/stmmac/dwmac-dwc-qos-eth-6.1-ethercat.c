...
static int dwc_eth_dwmac_probe(struct platform_device *pdev)
{
	...
	plat_dat = ec_stmmac_probe_config_dt(pdev, &stmmac_res.mac);
	if (IS_ERR(plat_dat))
	...
	ret = stmmac_ec_dvr_probe(&pdev->dev, plat_dat, &stmmac_res);
	if (ret)
		goto remove;
...
remove:
	data->remove(pdev);
remove_config:
	ec_stmmac_remove_config_dt(pdev, plat_dat);
...
static int dwc_eth_dwmac_remove(struct platform_device *pdev)
{
	...
	err = stmmac_ec_dvr_remove(&pdev->dev);
	if (err < 0)
	...
	ec_stmmac_remove_config_dt(pdev, priv->plat);

	return err;
}

static const struct of_device_id dwc_eth_dwmac_match[] = {
	{ .compatible = "ec_snps,dwc-qos-ethernet-4.10", .data = &dwc_qos_data },
	{ .compatible = "ec_nvidia,tegra186-eqos", .data = &tegra_eqos_data },
	{ .compatible = "ec_renesas,rzt2h-eqos", .data = &renesas_rzt2h_eqos_data },
	{ .compatible = "ec_renesas,rzn2h-eqos", .data = &renesas_rzt2h_eqos_data },
	{ }
};
...
static int __init dwc_eth_dwmac_init(void)
{
	int ret;
	ret = stmmac_init();
	if (ret) {
...
static void __exit dwc_eth_dwmac_exit(void)
{
	platform_unregister_drivers(drivers, ARRAY_SIZE(drivers));
	stmmac_exit();
}

module_init(dwc_eth_dwmac_init);
module_exit(dwc_eth_dwmac_exit);

MODULE_AUTHOR("Joao Pinto <jpinto@synopsys.com>");
MODULE_DESCRIPTION("Synopsys DWC Ethernet Quality-of-Service v4.10a driver (Ethercat Master Enabled)");
