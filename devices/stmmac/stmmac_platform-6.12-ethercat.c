...
#include "stmmac_platform-6.12-ethercat.h"
...
static struct device_node *stmmac_of_get_mdio(struct device_node *np)
{
	static const struct of_device_id need_mdio_ids[] = {
		{ .compatible = "ec_snps,dwc-qos-ethernet-4.10" },
		{},
	};
...
/**
 * ec_stmmac_probe_config_dt - parse device-tree driver parameters
...
ec_stmmac_probe_config_dt(struct platform_device *pdev, u8 *mac)
{
	...
	/* clock setup */
	if (!of_device_is_compatible(np, "ec_snps,dwc-qos-ethernet-4.10")) {
...
static void ec_devm_stmmac_remove_config_dt(void *data)
{
...
/**
 * ec_devm_stmmac_probe_config_dt
 * @pdev: platform_device structure
 * @mac: MAC address to use
 * Description: Devres variant of ec_stmmac_probe_config_dt().
 */
struct plat_stmmacenet_data *
ec_devm_stmmac_probe_config_dt(struct platform_device *pdev, u8 *mac)
{
	struct plat_stmmacenet_data *plat;
	int ret;

	plat = ec_stmmac_probe_config_dt(pdev, mac);
	if (IS_ERR(plat))
		return plat;

	ret = devm_add_action_or_reset(&pdev->dev,
				       ec_devm_stmmac_remove_config_dt, plat);
	if (ret)
		return ERR_PTR(ret);
...
#endif /* CONFIG_OF */
EXPORT_SYMBOL_GPL(ec_devm_stmmac_probe_config_dt);
...
static void ec_devm_stmmac_pltfr_remove(void *data)
{
 ...
 */
int ec_devm_stmmac_pltfr_probe(struct platform_device *pdev,
			    struct plat_stmmacenet_data *plat,
			    struct stmmac_resources *res)
{
	int ret;
	...
	return devm_add_action_or_reset(&pdev->dev, ec_devm_stmmac_pltfr_remove,
					pdev);
}
EXPORT_SYMBOL_GPL(ec_devm_stmmac_pltfr_probe);
