...
ec_stmmac_probe_config_dt(struct platform_device *pdev, const char **mac)
{
	...
	*mac = of_get_mac_address(np);
	...
	dma_cfg = devm_kzalloc(&pdev->dev, sizeof(*dma_cfg),
			       GFP_KERNEL);
	if (!dma_cfg) {
		ec_stmmac_remove_config_dt(pdev, plat);
		return ERR_PTR(-ENOMEM);
	}
...
/**
 * ec_stmmac_remove_config_dt - undo the effects of ec_stmmac_probe_config_dt()
 * @pdev: platform_device structure
 * @plat: driver data platform structure
 *
 * Release resources claimed by ec_stmmac_probe_config_dt().
 */
void ec_stmmac_remove_config_dt(struct platform_device *pdev,
			     struct plat_stmmacenet_data *plat)
{
...
#endif /* CONFIG_OF */
EXPORT_SYMBOL_GPL(ec_stmmac_probe_config_dt);
EXPORT_SYMBOL_GPL(ec_stmmac_remove_config_dt);
 ...
 */
int ec_stmmac_pltfr_remove(struct platform_device *pdev)
{
	struct net_device *ndev = platform_get_drvdata(pdev);
	struct stmmac_priv *priv = netdev_priv(ndev);
	struct plat_stmmacenet_data *plat = priv->plat;
	int ret = stmmac_ec_dvr_remove(&pdev->dev);
