...
#include "cpsw_ale-6.1-ethercat.h"
#include "cpsw_sl-6.1-ethercat.h"
#include "am65-cpsw-nuss-6.1-ethercat.h"
#include "am65-cpsw-switchdev-6.1-ethercat.h"
#include "k3-cppi-desc-pool-6.1-ethercat.h"
#include "am65-cpts-6.1-ethercat.h"
				     ...
				     u32 flow_idx)
{
	...
	if (port->ecdev) {
		ecdev_receive(port->ecdev, skb->data, pkt_len);
		new_skb = skb;
		goto ec_bypass;
	}
...
static void am65_cpsw_nuss_tx_cleanup(void *data, dma_addr_t desc_dma)
{
	...
	if (!am65_ndev_to_port(skb->dev)->ecdev)
		dev_kfree_skb_any(skb);
}
					   ...
					   int chn, unsigned int budget, bool *tdown)
{
	...
	while (true) {
		...
		total_bytes = skb->len;
		ndev = skb->dev;
		if (!am65_ndev_to_port(ndev)->ecdev) {
...
static void ec_poll(struct net_device *ndev)
{
	struct am65_cpsw_common *common = am65_ndev_to_common(ndev);
	int i, ret;
	...
	for (i = 0; i < common->tx_ch_num; i++) {
		...
		if (AM65_CPSW_IS_CPSW2G(common))
			am65_cpsw_nuss_tx_compl_packets_2g(common, i,
							   budget, &tdown);
		else
			am65_cpsw_nuss_tx_compl_packets(common, i,
							budget, &tdown);
	}

	for (i = AM65_CPSW_MAX_RX_FLOWS - 1; i >= 0; i--) {
		do {
			ret = am65_cpsw_nuss_rx_packets(common, i);
		} while (ret == 0);
	}
}
					 ...
					 phy_interface_t interface)
{
	...
	if (!port->ecdev)
		...
		ecdev_set_link(port->ecdev, false);
}
				       ...
				       int duplex, bool tx_pause, bool rx_pause)
{
	...
	if (!port->ecdev)
		...
		ecdev_set_link(port->ecdev, true);
}
...
static int am65_cpsw_nuss_register_ndevs(struct am65_cpsw_common *common)
{
	...
	for (i = 0; i < common->port_num; i++) {
		...
		port->ecdev = ecdev_offer(port->ndev, ec_poll, THIS_MODULE);
		if (!port->ecdev) {
...
MODULE_DESCRIPTION("Ethercat-capable TI AM65 CPSW Ethernet driver");
